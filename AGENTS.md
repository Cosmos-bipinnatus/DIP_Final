...
| **自訂與空間濾波器** | `convolution_filter` | 已實作 (Consolidated) | 支援自訂 3x3 與 5x5 濾波核心係數、除數與偏移量。C# 前端具備實質灰階檢查與彩色影像警告攔截，網格控制項自適應顯示不溢出視窗。其餘邊緣與平滑濾波器（Sobel、Canny）底層已重構為調用此通用卷積函式。 |

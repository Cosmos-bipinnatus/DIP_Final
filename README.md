...
1. **影像轉灰階 (`encode_gray` & `ConvertTo8bppGrayscale`)**
   * **功能:** 將 BGR/ARGB 彩色影像以標準加權平均法轉換為灰階：Y = 0.299*R + 0.587*G + 0.114*B。
   * **子選項支援:**
     * **灰階顯示預覽 (24-bit 輸出)**：調用 C++ 核心 `encode_gray` 將 BGR 通道設為相同灰階值。輸出格式仍為 24-bit / 32-bit，用於一般預覽。
     * **轉換為灰階圖像 (8-bit 輸出)**：調用 C# 輔助 `ConvertTo8bppGrayscale`，輸出標準 Format8bppIndexed 格式與 256 色漸變調色盤，供後續特殊演算法使用。
     * **直方圖偏差說明**：因儲存格式（直接儲存 vs 索引調色盤）不同，受 GDI+ 色彩空間映射、捨入誤差與調色盤對齊影響，兩者的直方圖數值在部分影像上可能產生極微小偏差。
   * **參數:**
     * `int* f`: 輸入影像一維像素陣列指標
     * `int w`, `int h`: 影像寬度、高度
     * `int d`: 深度通道數 (3)
...
2. **位元切面提取 (`bit_plane_slice`)**
   ...
   * **UI 整合與自動 8-bit 灰階轉換降級機制:**
     * **合併式 UI 視窗**: 將滑桿 TrackBar、二值化 CheckBox 與預覽 PictureBox 整合在同一個 `BitPlaneSliceForm` MDI 子視窗中。
     * **自動 8-bit 灰階轉換 (代替 ALERT 報錯)**: 對於位元平面切片、手動與大津二值化、自訂空間濾波、Canny 及 2D 霍夫直線/圓形偵測，若輸入影像為彩色或非標準 8-bit 灰階，系統將自動將其轉換為標準的 8-bit 灰階影像並新建一個子視窗輸出，再在此 8-bit 灰階影像上執行選定演算法，徹底保障使用者體驗流暢。
...
* **核心對比度公式 (原點移至中間值 128):**
  `g(x,y) = clamp( alpha * (f(x,y) - 128) + 128 + beta )`
...
2. **影像旋轉 (`rotate_image`)**
   ...
   * **參數:**
    ...
    * **UI 整合與歷史記憶機制:**
      * **自訂背景色記憶與共享**: 使用靜態變數 `lastCus### 3.2 功能測試流程 (Testing Flow)
1. **啟動程式:** 執行 `DIP/bin/x86/Debug/DIP.exe`。
2. **開啟影像:** 點擊選單 `File` -> `Open`，選擇任意長方形或正方形的 BMP 影像（如標準 Lena 或自訂長方形圖片）。
3. **直方圖側邊欄（永久顯示）與參數持久化驗證:** 影像開啟後，右側 Sidebar 將永久顯示（且選單上無切換按鈕），自動呈現直方圖、影像基本數據、目前影像套用的演算法參數與詳細的原理說明：
   * **直方圖分層**：彩色影像分層顯示藍、綠、紅三通道直方圖（各高度 120）；灰階影像自適應收縮為單一深灰色直方圖（高度 200）。
   * **影像生成設定與原理說明 (Parameters & Docs)**：
     * 對於**作用中的調整預覽視窗**，直方圖下方區域會顯示 `[Previewing...]` 或動態反映當下控制項或滑桿的值。
     * 對於**已確認輸出的影像視窗**，該區域會持久化顯示該影像所使用的核心參數（如手動閾值二值化的 T、旋轉角度、濾波核心大小等），並附帶中等詳細度的教育性演算法原理介紹。
     * 切換不同的 MDI 子視窗時，直方圖、統計數據、參數與原理說明會即時同步切換。
4. **測試 Bit-Plane 滾動預覽:** 點擊 `Basic` -> `Bit Plane Slicing`，在彈出的滑桿視窗中從 0 到 7 拖曳。左側預覽視窗應會**實時同步**呈現各個 bitplane 的二值化效果。
5. **測試亮度對比與 Gamma 調整:** 在 `Basic` 選單中點擊 `Brightness, Contrast & Gamma Adjustment`：
...
6. **測試幾何旋轉與背景色記憶:** 點擊 `Image Rotation`，設定角度為 $45$ 度，選擇 `Bilinear` 插值。
   * 勾選「自訂背景色」，若為首次開啟會彈出色彩調色盤，請選擇一個非黑色（如綠色）的顏色。
   * 建立一幅背景為該自訂色彩填滿的新影像。
   * 關閉該旋轉子視窗，再次點擊 `Image Rotation`，再次勾選「自訂」，驗證系統直接載入剛才選擇的綠色，未再次彈出調色盤（若點擊「選擇顏色」按鈕仍可手動變更顏色）。
7. **測試影像幾何縮放:** 點擊 `Scaling` -> `Nearest Neighbor` 或 `Bilinear Interpolation`：
   * 彈出 `Scale Form`，此時預覽滑桿完美位於正中央，標示為 100%。
   * 向左拖曳至 10% 或向右拖曳至 500%，選擇插值模式（最近鄰/雙線性），點擊確定後輸出新影像。
   * 驗證輸出影像的長寬是否按比例縮放，且雙線性插值無鋸齒崩潰。
8. **測試手動門檻二值化與 Alert 攔截:** 點擊 `Segmentation & Thresholding` -> `Manual Thresholding`：
   * 若目前為彩色影像，應跳出 Alert 警示 MessageBox，提示並攔截阻擋。
   * 若為灰階影像（或先執行 `RGB to Gray` 轉為灰階後），則順利打開 `ManualThresholdForm`。
   * 拖曳閥值滑桿（預設 128），上方影像應即時預覽二值化，且右側直方圖也同步動態更新。點擊確定後輸出二值化影像。
9. **測試 2D 霍夫線/圓偵測:** 點擊 `Hough Detection` 下的 `Hough Line Detection` 或 `Hough Circle Detection` 進行檢測，結果影像應會直接以指定繪圖顏色圈出偵測到的直線與圓圈。�� UI 設計規範「上方為影像預覽 PictureBox，下方為參數調整操作 Panel」之 MDI 整合式表單。
...
1. **Sobel 邊緣偵測 (`detect_sobel`)**
   * **功能:** 利用一階導數的 $3 \times 3$ 卷積算子計算影像的水平與垂直梯度振幅，提取影像的顯著邊緣。
   * **參數:**
     * `int* f`: 輸入影像一維像素陣列指標
     * `int w`, `int h`: 影像寬度、高度
     * `int d`: 深度通道數（`1` 代表灰階，`3` 代表 BGR 彩色）
     * `int* g`: 輸出影像陣列指標
   * **優化與防護機制:**
     * **免拷貝效能優化**: 移除了多餘的輸入至輸出全圖記憶體拷貝，內部與邊界像素直接運算覆蓋，大幅提高大影像處理下的快取命中率與執行速度。
     * **邊界安全補零**: 在 C++ 核心對最外層 1 像素寬的邊界進行 `0` 填入，解決了卷積模板在鄰域取值時的越界問題。
     * **自動灰階相容**: C++ 層支援三通道 BGR 自動以 BT.601 公式先轉灰階後計算，保障了彩色與灰階影像的相容性。
2. **Canny 邊緣檢測算子 (`detect_canny`)**
   * **功能:** 包含高斯平滑、梯度強度/方向計算、非極大值抑制 (NMS) 與雙閾值滯後及連動追蹤 4 步驟。
   * **參數:**
     * `int* f`: 輸入影像一維像素陣列指標
     * `int w`, `int h`: 影像寬度、高度
     * `int d`: 深度通道數
     * `int* g`: 輸出二值化邊緣影像陣列指標
     * `double lowThresh`: 低門檻值 (0.0 ~ 255.0)
     * `double highThresh`: 高門檻值 (0.0 ~ 255.0)
   * **UI 整合與安全防護機制:**
     * **合併式 UI 預覽視窗 (`CannyForm`)**: 採用「上方為影像預覽 PictureBox，下方為參數調整操作 Panel」設計，拖曳「低門檻」與「高門檻」雙滑桿時，即時調用 C++ 演算法更新預覽並連動直方圖。滑桿值自動限制在 $T_{low} \le T_{high}$ 的合法區間。
     * **實質灰階檢查**: 前端限制僅能處理實質灰階影像，對彩色影像進行 Alert 攔截警告，提示先進行 RGB 轉灰階。
3. **霍夫直線檢測 (`detect_lines_hough`)**
   * **功能:** 邊緣化後進行參數空間 (rho, theta) 累加投票，並在輸出影像上以指定顏色繪製疊加直線。
   * **參數:**
     * `int* f`: 輸入影像陣列指標
     * `int w`, `int h`: 影像寬高
     * `int d`: 深度通道數
     * `int* g`: 輸出影像陣列指標
     * `int houghThreshold`: 投票門檻值
     * `int lineR`, `int lineG`, `int lineB`: 繪圖顏色之 RGB 通道值
   * **UI 整合與相容性修復機制:**
     * **整合式 UI 預覽與自適應大圖縮放**: 採用「上方為影像預覽 PictureBox，下方為參數調整操作 Panel」設計。拖曳「累加器門檻」滑桿時，即時運算更新。若輸入大圖，自動等比例縮放預覽區域，並在 MouseMove 事件中實施縮放比例座標還原，精確對應大圖真實點坐標與 RGB 值。
     * **自訂繪線顏色調色盤**: 下方控制區內建調色盤方格。僅點選色彩預覽方格時才會彈出選色對話框。自訂 RadioButton 與確定關閉時不彈出，保障操作手感。
     * **非對齊點陣圖格式安全轉換**: 針對 1bpp、4bpp 等非位元組對齊的點陣圖（如 `small-squares.bmp`），在 `dyn_bmp2array` 階段先自動轉換為標準 of `Format24bppRgb` 以計算正確 Stride，完全避免在 C++ 及 C# 處理時的記憶體越界與 Access Violation 崩潰。
     * **實質灰階檢查 (ALERT 攔截)**: C# 前端強行進行真灰階攔截，非標準 8bpp Indexed 灰階影像時跳出 Alert 阻擋，且警告文字中不引導使用者去使用 `RGB to Gray` 轉換（因為轉換後為 24-bit，不符合 8bpp 格式限制）。
4. **霍夫圓形檢測 (`detect_circles_hough`)**
   * **功能:** 邊緣化後沿著梯度方向及其偏轉方向進行 3D 參數空間 (a, b, r) 累加投票，並以指定顏色圓圈疊加標記。
   * **參數:**
     * `int* f`: 輸入影像陣列指標
     * `int w`, `int h`: 影像寬高
     * `int d`: 深度通道數
     * `int* g`: 輸出影像陣列指標
     * `int rMin`, `int rMax`: 偵測半徑範圍
     * `int houghThreshold`: 投票門檻值
     * `int lineR`, `int lineG`, `int lineB`: 繪圖顏色之 RGB 通道值
   * **核心優化與 UI 機制:**
     * **投票運算極速查表最佳化**: 使用梯度單位向量直接計算投影圓心坐標，並採用 cos(10度) = 0.9848 與 sin(10度) = 0.1736 的常數乘加做正負 10 度方向偏轉。已徹底移除 C++ 內部 atan2, cos, sin 等浮點三角函數呼叫，拖曳滑桿即時計算流暢。
     * **門檻值預設定位與置中**: 滑桿預設值設為 50，位於滑桿正中央且圓偵測靈敏度良好。
     * **縮放預覽與緊湊排版防遮擋**: 預覽 Form 針對大圖實行 Zoom 縮放且滑鼠座標正確映射；下方 Panel 垂直間隔全面降為 35px，總高度縮減至 225px，徹底解決視窗底部按鈕被擠壓遮擋的問題。
     * **懸浮檢視資料分流**: 在 MDI 輸出視窗取消勾選「融入原始影像」時，畫面上仍然維持線條顯示以便檢視，但點選複製或另存圖片時，則會分流輸出乾淨、不帶任何標記線的原始影像。
...
3. **直方圖側邊欄（永久顯示）與參數持久化驗證:** 影像開啟後，右側 Sidebar 將永久顯示（且選單上無切換按鈕），自動呈現直方圖、影像基本數據、目前影像套用的演算法參數與詳細的原理說明：
   * **直方圖分層**：彩色影像分層顯示藍、綠、紅三通道直方圖（各高度 120）；灰階影像自適應收縮為單一深灰色直方圖（高度 200）。
   * **影像生成設定與原理說明 (Parameters & Docs)**：
     * 對於**作用中的調整預覽視窗**，直方圖下方區域會顯示 `[即時預覽調整中]`，並動態反映當下控制項或滑桿的值。
     * 對於**已確認輸出的影像視窗**，該區域會持久化顯示該影像所使用的核心參數（如手動閾值二值化的 $T$、旋轉角度、濾波核心大小等），並附帶中等詳細度的教育性演算法原理介紹。
     * 切換不同的 MDI 子視窗時，直方圖、統計數據、參數與原理說明會即時同步切換。

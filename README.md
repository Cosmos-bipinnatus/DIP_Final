...
2. **影像旋轉 (`rotate_image`)**
   ...
   * **UI 整合與歷史記憶機制:**
     * **自訂背景色記憶**: 使用靜態 `static` 變數來儲存使用者最後選取的自訂顏色，能保證其不會隨著 `RotateImageForm` 關閉銷毀而遺失。
     * **首開自動彈出調色盤**: 若為首次打開且無任何歷史選色紀錄時，使用者勾選「自訂」會自動彈出 ColorDialog 調色盤；若已有歷史紀錄，則直接載入歷史顏色，不主動打擾使用者。

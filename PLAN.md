...
#### 2.2.3 自訂背景色記憶與首開機制
* **歷史紀錄記憶**: 系統提供全域靜態變數 `static Color? lastCustomBgColor` 來記住使用者最後一次為自訂背景所選取的顏色。此變數不隨旋轉對話框關閉而重置。
* **首次選色彈出**: 當使用者第一次勾選「自訂」RadioButton 且系統中無任何選色紀錄（即靜態變數為 `null`）時，系統會自動彈出 `ColorDialog` 供使用者進行選色；之後若已有儲存的歷史紀錄，勾選時會直接讀取該色彩以簡化工作流程。若需變更顏色，使用者可隨時點擊旁邊的「選擇顏色」按鈕。

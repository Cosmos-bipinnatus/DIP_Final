...
### 2.7 Canny 邊緣偵測 (Canny Edge Detection)
Canny 邊緣檢測演算法是一種最優化的邊緣檢測方法，其規格與影像處理流水線如下：

1. **高斯平滑濾波降噪**：使用 3x3 高斯模板對影像進行卷積以消除高頻雜訊。
2. **計算梯度強度與方向**：使用 Sobel 模板計算水平與垂直方向的一階導數 $G_x$ 與 $G_y$，求得梯度強度 $M = \sqrt{G_x^2 + G_y^2}$ 與方向 $\theta = \text{atan2}(G_y, G_x)$。
3. **非極大值抑制 (NMS)**：將梯度方向量化為 0, 45, 90, 135 度四個主要方向，比較鄰域在梯度正交方向上的強度值。若當前像素不是局部極大值，則抑制歸零，實現單像素寬度的「邊緣削薄」。
4. **雙門檻值分類與滯後追蹤**：設定高門檻 $T_{high}$ 與低門檻 $T_{low}$。梯度強度大於 $T_{high}$ 的像素分類為強邊緣，小於 $T_{low}$ 的排除。介於兩者之間的弱邊緣，只有當其在 8 鄰域中與強邊緣相連通時，才被判定為真實邊緣予以保留。
5. **C# 整合預覽視窗 (`CannyForm`)**：採用「上方預覽 PictureBox，下方參數 Panel 控制」版面規範。當拖曳雙門檻滑桿時即時運算更新，自動維護 $T_{low} \le T_{high}$ 的一致性，並提供彩色影像 Alert 攔截阻擋與直方圖實時更新。
...
* **C++ Signature:**
  ...
  * **濾波器核心規格與演算法設計：**
    * **1. 均值平滑 (Mean Filter)**
      - 核心：3x3 全為 1.0 的矩陣。
      - 預設值：除數 (divisor) = 9.0，偏移量 (offset) = 0.0。
      - 運作工作流：將中心像素與其周圍 8 鄰域像素的值加總，除以 9.0，使影像亮度平滑模糊，藉此抑制高頻雜訊。
    * **2. 高斯平滑 (Gaussian Filter)**
      - 核心：3x3 正態分佈權重矩陣：[1, 2, 1; 2, 4, 2; 1, 2, 1]。
      - 預設值：除數 (divisor) = 16.0，偏移量 (offset) = 0.0。
      - 運作工作流：利用鄰域加權求和，越靠近中心的像素權重越高，除以權重總和 16.0，提供更自然的去噪模糊，對影像輪廓更溫和。
    * **3. 拉普拉斯銳化 (Laplacian - 8鄰域)**
      - 核心：3x3 核心：[-1, -1, -1; -1, 8, -1; -1, -1, -1]。
      - 預設值：除數 (divisor) = 1.0，偏移量 (offset) = 0.0。
      - 運作工作流：二階微分差分，突出亮度突變（高頻邊緣），所得的高頻差分疊加回影像中，可大幅增強影像細節的對比度。
    * **4. 高斯-拉普拉斯算子 (LoG)**
      - 核心：5x5 核心：[0, 0, -1, 0, 0; 0, -1, -2, -1, 0; -1, -2, 16, -2, -1; 0, -1, -2, -1, 0; 0, 0, -1, 0, 0]。
      - 預設值：除數 (divisor) = 1.0，偏移量 (offset) = 128.0。
      - 運作工作流：結合高斯模糊去噪與拉普拉斯邊緣偵測，加入 128.0 偏移量可將正負二階導數變化可視化顯示。
    * **5. 反銳化遮罩與高提升濾波 (High-boost)**
      - 合成核心：3x3 核心，中心為 8 + (A - 1.0) * 9，其餘為 -1，其中 A 為使用者自訂比例 (1.0~3.0)。
      - 預設值：除數 (divisor) = 1.0，偏移量 (offset) = 0.0。
      - 運作工作流：以原圖減去低通模糊的差分圖（細節遮罩）乘以權重後加回原圖，顯著增強邊緣細緻度與銳化質感。
...
* **C++ Signature:**
  ...
  // 霍夫直線檢測 (邊緣化後投票，並在輸出圖 g 上以自訂顏色疊加繪製直線)
  ...
  // 霍夫圓形檢測 (偵測圓形，並在輸出圖 g 上以自訂顏色繪製圓形邊緣)
...
| **自訂與空間濾波器** <br>(`convolution_filter`) | 1. 含噪影像 (椒鹽或高斯雜訊) <br> 2. 模糊或需銳化的影像 <br> 3. 自訂濾波核心需求 | **平滑/降噪：** `ISample01/Lena256_salt.bmp` (椒鹽雜訊), `ISample01/Pepper256.BMP` <br> **銳化/邊緣：** `ISample02/cameraman.bmp` | 驗證均值/高斯濾波器的去噪效果，以及拉普拉斯/LoG 核心的銳化邊緣提取。同時需驗證邊界處理為 Zero Padding，觀察影像邊緣是否會變黑或有平滑過渡。並驗證自訂 3x3 與 5x5 核心、除數、偏移量的濾波效果與彩色影像 Alert 攔截。 |
